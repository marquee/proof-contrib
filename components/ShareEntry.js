...
var Classes, EMAIL, FACEBOOK, HORIZONTAL, ICONS, LINKEDIN, PINTEREST, POCKET, React, SERVICE_PROPER_NAMES, TWITTER, UglifyJS, VERTICAL, WINDOW_OPTIONS, buildLinkFor, k, num_instances, v;
...
ICONS = Object.freeze({
  ...
  pinterest: React.createElement("svg", {
    "className": '_Icon',
    "width": '1em',
    "height": '1em',
    "viewBox": '0 0 16 20',
  ...
  pocket: React.createElement("svg", {
    "className": '_Icon',
    "width": '1em',
    "height": '1em',
    "viewBox": '0 0 20 18',
...
buildLinkFor = function(entry, service) {
  var body, cover, link, summary, title;
...
module.exports = React.createClass({
  ...
  propTypes: {
    services: function(props, prop_name, component_name) {
      var i, len, ref, service;
      ref = props[prop_name];
      for (i = 0, len = ref.length; i < len; i++) {
        service = ref[i];
        if (service !== LINKEDIN && service !== FACEBOOK && service !== TWITTER && service !== PINTEREST && service !== POCKET && service !== EMAIL) {
          return new Error('');
        }
      }
    },
    entry: React.PropTypes.shape({
      ...
      cover_image: React.PropTypes.oneOfType([React.PropTypes.string, React.PropTypes.object]),
  ...
  getInitialState: function() {
    num_instances += 1;
    return {
      id: this.props.entry.id + "-ShareEntry--" + num_instances
    };
  },
  render: function() {
    var _cx;
    _cx = new Classes('ShareEntry', this.props.className);
    _cx.set('layout', this.props.layout);
    _cx.add('greyscale', this.props.greyscale);
    _cx.add('border', this.props.border);
    return React.createElement("div", {
      "className": _cx,
      "id": this.state.id
    }, (this.props.label ? React.createElement("span", {
      "className": '_Label'
    }, this.props.label) : void 0), React.createElement("div", {
      "className": '_Services'
    }, this.props.services.map((function(_this) {
      return function(service) {
        var _popup, cx, service_id;
        _popup = service !== EMAIL;
        cx = new Classes('_ShareLink');
        cx.set('service', service);
        cx.add('icon', _this.props.icon);
        service_id = _this.state.id + "--" + service;
        return React.createElement("a", {
          "href": buildLinkFor(_this.props.entry, service),
          "className": cx,
          "target": (_popup ? '_blank' : null),
          "tabIndex": 0,
          "key": service,
          "aria-label": "Share " + _this.props.entry.title + " on " + SERVICE_PROPER_NAMES[service],
          "id": service_id
        }, (_this.props.icon ? ICONS[service] : React.createElement("span", {
          "className": '_Label'
